...
class NameService
{
    ...
    private static $choseongToOhaengMap = [
        'ㄱ' => '목', 'ㅋ' => '목', 'ㄲ' => '목', // 木 (나무)
        'ㄴ' => '화', 'ㄷ' => '화', 'ㄹ' => '화', 'ㅌ' => '화', 'ㄸ' => '화', // 火 (불)
        'ㅇ' => '토', 'ㅎ' => '토', // 土 (흙)
        'ㅅ' => '금', 'ㅈ' => '금', 'ㅊ' => '금', 'ㅆ' => '금', 'ㅉ' => '금', // 金 (쇠)
        'ㅁ' => '수', 'ㅂ' => '수', 'ㅍ' => '수', 'ㅃ' => '수', // 水 (물)
    ];
    ...
    // =================================================================================
    // 이름 풀이 및 작명을 위한 계산 메소드 (From NameController)
    // =================================================================================
    ...
    public function filterGoodStrokes($ohengs, $gender, $hanjano1): array
    {
        ...
        $result = [];
        for ($j = 1; $j < 30; $j++) {
            if ($this->getOhaengByStrokes($j) !== $oheng2) {
                continue;
            }
            for ($k = 1; $k < 30; $k++) {
                if ($this->getOhaengByStrokes($k) !== $oheng3) {
                    continue;
                }
