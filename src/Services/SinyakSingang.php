...
class SinyakSingang
{
    ...
    private function calculateAllyScores(): array
    {
        ...
        $jiji_pillars = ['year', 'month', 'day'];
        if ($this->saju->hourKnown) {
            $jiji_pillars[] = 'hour';
        }

        // 지지(地支)의 아군 점수 계산
        foreach ($jiji_pillars as $pillar) {
            $jijiElement = $this->convertCharToOhaeng($this->saju->get_e($pillar));
        ...
        $cheongan_pillars = ['year', 'month'];
        if ($this->saju->hourKnown) {
            $cheongan_pillars[] = 'hour';
        }
        foreach ($cheongan_pillars as $pillar) {
            $cheonGanElement = $this->convertCharToOhaeng($this->saju->get_h($pillar));
