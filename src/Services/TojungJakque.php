...
class TojungJakque
{
    ...
    public $age;
    public $now;
    public $this_year; // $now_lunar_year,
    ...
    */
    public function create($year = null)
    {
        ...
        list($year, $month, $day)  = explode('-', $this->saju->lunar);
        $this_year_ymd = $this->this_year.'-'.$month.'-'.$day;
        $this->now = Saju::ymdhi($this_year_ymd)->sl('lunar')->create();


        ## 상괘
        $this->que[0] = $this->sangque($this->saju);
        ## 중괘
        $this->que[1] = $this->jungque($this->saju);
        ## 하괘
        $this->que[2] = $this->haque($this->saju);
    ...
    private function sangque($manse)
    {
        ...
        $this->age = $this->this_year - $manse_year + 1; // 올해의 한국 나이
