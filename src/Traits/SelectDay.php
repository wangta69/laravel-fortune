...
trait SelectDay
{
     ...
     * 월지에 따른 길한 일지를 매핑합니다.
     */
    protected function _cheonmun($month_e, $day_e, &$titles, &$scores)
    {
        // 월지별 천문개일 일지 지도
        $cheonmun_map = [
            '子' => '巳', '丑' => '午', '寅' => '未', '卯' => '申',
        ...
        if (isset($cheonmun_map[$month_e]) && $cheonmun_map[$month_e] === $day_e) {
            $titles['cheonmun'] = [
                'ko' => '천문개일',
                'desc' => '하늘의 문이 열리는 길일로 새로운 시작과 기도의 성취에 길함',
                'type' => 'gilsin',
            ];
            $scores['cheonmun'] = 20; // 높은 점수 부여 (골든데이 후보)
        }
    }
 ...
 .방녀신 : 신부에게 나쁜 달
    ...
    public function _cheonsa($month_e, $day_he, &$titles, &$scores)
    {
        ...
        if (isset($cheonsa_map[$month_e]) && $cheonsa_map[$month_e] === $day_he) {
            $titles['cheonsa'] = [
                'ko' => '천사성',
                'ch' => '天赦星',
                'desc' => '하늘이 모든 허물을 용서하는 날로, 만사에 길한 최고의 길일입니다.',
                'type' => 'gilsin',
            ];
            $scores['cheonsa'] = 50; // 최상급 길일이므로 점수를 높게 책정
        }
    }
    ...
    /**
     * 공망일: 결과가 헛되고 실속이 없는 날
     */
    public function _gongmang($owner_ilju, $day_e, &$titles, &$scores)
    {
        $gongmang_map = [
        ...
        if (isset($gongmang_map[$owner_ilju]) && in_array($day_e, $gongmang_map[$owner_ilju])) {
            $titles['gongmang'] = ['ko' => '공망일', 'desc' => '일의 결과가 비고 헛될 수 있으니, 새로운 시작에는 매우 부적합한 날입니다.', 'type' => 'hyungsal'];
            $scores['gongmang'] = -100;
        }
    }

    /**
     * 12포태법 재물운 길일: 대표자 일간 기준 재물 기운이 강한 날
     */
    public function _jaemul_woonseong($owner_ilgan, $day_e, &$titles, &$scores)
    {
        // 십신(十神)을 기준으로 재성(財星)의 천간을 찾습니다.
        $jaeseong_h = '';
        if (in_array($owner_ilgan, ['甲', '乙'])) {
            $jaeseong_h = '戊';
        } // 목극토
        if (in_array($owner_ilgan, ['丙', '丁'])) {
            $jaeseong_h = '庚';
        } // 화극금
        if (in_array($owner_ilgan, ['戊', '己'])) {
            $jaeseong_h = '壬';
        } // 토극수
        if (in_array($owner_ilgan, ['庚', '辛'])) {
            $jaeseong_h = '甲';
        } // 금극목
        if (in_array($owner_ilgan, ['壬', '癸'])) {
            $jaeseong_h = '丙';
        } // 수극화

        // 재성의 12운성이 길한지 봅니다. (포태법)
        $woonseong_map = [
        ...
        // 음간(陰干)은 양간(陽干)을 따릅니다.
        $woonseong_map['乙'] = $woonseong_map['甲'];
        $woonseong_map['丁'] = $woonseong_map['丙'];
        $woonseong_map['己'] = $woonseong_map['戊'];
        $woonseong_map['辛'] = $woonseong_map['庚'];
        $woonseong_map['癸'] = $woonseong_map['壬'];

        if (isset($woonseong_map[$jaeseong_h][$day_e])) {
            $woonseong_name = $woonseong_map[$jaeseong_h][$day_e];
            // 장생, 관대, 건록, 제왕에 높은 가산점 부여
            if (in_array($woonseong_name, ['장생', '관대', '건록', '제왕'])) {
                $titles['jaemul_woon'] = ['ko' => '재물 '.$woonseong_name.'일', 'desc' => '대표자의 재물 기운이 매우 왕성한 날로, 개업에 최적입니다.', 'type' => 'gilsin'];
                $scores['jaemul_woon'] = 40;
            }
        }
    }
}
